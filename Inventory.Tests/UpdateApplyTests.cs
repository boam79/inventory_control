...
public class UpdateApplyTests
{
    ...
    {
        const string json = """
            {"Assets":[{"PackageId":"SpringClinic.Inventory","Version":"1.0.24","Type":"Full","FileName":"SpringClinic.Inventory-1.0.24-full.nupkg","SHA256":"ABC"}]}
            """;
        var offer = UpdateChecker.ParseVelopackWinFeed(json);
        Assert.True(offer.Found);
        Assert.Equal("v1.0.24", offer.VersionTag);
        Assert.Equal(UpdateChecker.SetupDownloadUrl("v1.0.24"), offer.PackageUrl);
