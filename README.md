...
업데이트는 GitHub Releases의 zip/exe와 `.sha256`이 있을 때만 내려받아 해시를 검사하고, 실패하면 **지금 실행 중인 프로그램은 그대로** 둡니다. Velopack 설치 래퍼는 도입하지 않았습니다(배포 피드에 Velopack 패키지가 없고, 실행 중 외부 설치기를 받지 않기 위함).
